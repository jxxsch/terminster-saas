...
| 2026-02-12 | **Terminsuche im Dashboard:** Suchfeld im Kalender-Header (Desktop immer sichtbar, Mobile Lupe-Icon). Suche nach Kundenname/Telefon in nächsten 5 Wochen. Dropdown mit Treffern, Klick navigiert zur richtigen Woche/Tag. Treffer mit goldenem Ring hervorgehoben, Nicht-Treffer gedimmt. Keyboard-Navigation (ESC/Pfeiltasten/Enter). `useSearchAppointments` SWR-Hook. | `components/dashboard/AppointmentSearch.tsx` (NEU), `hooks/swr/use-dashboard-data.ts`, `app/(internal)/dashboard/page.tsx`, `components/dashboard/WeekView.tsx`, `components/dashboard/BarberWeekView.tsx`, `components/dashboard/AppointmentSlot.tsx` |
